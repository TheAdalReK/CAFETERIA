document.addEventListener('DOMContentLoaded', () =>{
    ...
    if(cookie != null){
        ...
        console.log(count);
        document.querySelector('.btn-carrito').innerHTML = `(${count}) Pedido`;
    }
});
...
function actualizarCarritoUI(){
    fetch('https://cafeteriamineral.000webhostapp.com/api/carrito/api-carrito.php?action=mostrar')
            ...
            enviarPedido = `
                <div class='enviarPedido'>
                    <form action='enviarPedido.php' method='POST'>
                        ...
                        <input type='text' name='nombre' placeholder="A Nombre del cliente" />
...
function eliminarItems() {
    fetch('https://cafeteriamineral.000webhostapp.com/api/carrito/api-carrito.php?action=removeAll', {
        method: 'POST'
    })
    .then(response => response.json())
    .then(data => {
...
function removeItemFromCarrito(id){
    fetch('https://cafeteriamineral.000webhostapp.com/api/carrito/api-carrito.php?action=remove&id=' + id)
    .then(res => res.json())
    .then(data =>{
...
function addItemToCarrito(id){
    fetch('https://cafeteriamineral.000webhostapp.com/api/carrito/api-carrito.php?action=add&id=' + id)
    .then(res => res.json())
    .then(data =>{
        console.log(data.status);
        actualizarCarritoUI();
